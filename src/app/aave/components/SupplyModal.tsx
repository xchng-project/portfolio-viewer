...
import {bigDecimal, chainId as aaveChainId, evmAddress, useSupply} from '@aave/react'
...
import type {SupplyOption} from './SupplyOptionsTable'

interface Props {
    isOpen: boolean
    onClose: () => void
    asset: SupplyOption | null
}

const SupplyModal = ({asset, isOpen, onClose}: Props) => {
    ...
    const isExceedingBalance = amount !== '' ? parseFloat(amount) > parseFloat(asset.balance) : false
    ...
    const confirmHandler = async () => {
        if (!asset.address || !asset.market || !walletAddress) {
            return
        }
        ...
        const result = await supply({
            ...
            onBehalfOf: evmAddress(walletAddress),
        }).andThen((plan) => {
    ...
    return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        ...
        >
            <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center">
                <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Supply {asset.symbol}</h3>
                <button
            ...
            <div className="p-6 space-y-4">
                <div className="space-y-4">
                    ...
                    <div className="relative">
                        <input
                            ...
                            placeholder="0.00"
                            className={`w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-900 border ${isExceedingBalance ? 'border-red-500' : 'border-zinc-200 dark:border-zinc-800'} rounded-xl text-zinc-900 placeholder:text-zinc-400 dark:text-zinc-100 dark:placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600 transition-all`}
                        />
                ...
                <button
                    ...
                    disabled={!amount || parseFloat(amount) <= 0 || isExceedingBalance || supplying.loading || sending.loading || !!txHash}
                ...
                >
                    {(supplying.loading || sending.loading) && (
                        <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                    )}
                    {txHash ? 'Supply Successful' : (supplying.loading || sending.loading ? 'Processing...' : 'Confirm Supply')}
                </button>
                {txHash && (
